...
@torch.no_grad()
def label_generator_dbscan_context_eps(cfg, features, features_parts=None, cuda=True, indep_thres=None, all_inds=None, epoch=0, eps=None, **kwargs):
    assert cfg.PSEUDO_LABELS.cluster == "dbscan_context_eps"
    ...
    if features_parts is not None:
        dist_weight = 0.3
        print("Parts weight is: {}".format(dist_weight))
        dist_p1 = build_dist(cfg.PSEUDO_LABELS, features_parts[0], verbose=True)
        dist_p2 = build_dist(cfg.PSEUDO_LABELS, features_parts[1], verbose=True)
        dist = dist*(1-2*dist_weight) + dist_p1*dist_weight + dist_p2*dist_weight
    ...
    if len(eps) == 1:
        ...
        labels_normal, _, num_classes, tmp_eps = process_label_with_context_save(labels_normal, centers_normal, features, all_inds, num_classes, epoch, frac)
