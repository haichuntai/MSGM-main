...
class LabelGenerator(object):
    ...
    @torch.no_grad()
    def __call__(self, cuda=True, memory_features=None, memory_features_parts=None, ca=None, kmeans_split=False, image_inds=None, epoch=0, eps=None, **kwargs):
        ...
        for idx, (data_loader, dataset) in enumerate(
            ...
            if self.cfg.PSEUDO_LABELS.norm_feat:
                ...
                if memory_features_parts != None:
                    if isinstance(memory_features_parts, list):
                        memory_features_parts = [F.normalize(f, p=2, dim=1) for f in memory_features_parts]
                    else:
                        memory_features_parts = F.normalize(memory_features_parts, p=2, dim=1)
            ...
            if self.rank == 0:
                ...
                if self.cluster_type == 'dbscan_context_eps' or self.cluster_type == 'dbscan_context_eps_all' or self.cluster_type == 'dbscan_context_eps_all_weight':
                    labels, centers, num_classes, indep_thres, tmp_eps = self.__factory[
                        ...
                        self.cfg,
                        all_features,
                        features_parts=memory_features_parts,
                        ca=ca,
                    ...
                    # if epoch == 0:
                    #     epoch0_saved = torch.load(
                    #         '/storage/disk1/wjb/CGPS/jobs/cuhk/work_dirs/model_epoch0_pospos_flattenfcreid')
                    #     labels = epoch0_saved['labels_normal']
                    #     centers = epoch0_saved['centers']
                    #     num_classes = epoch0_saved['num_classes']
                    #     indep_thres = epoch0_saved['indep_thres']
                    # else:
    ...
    def print_label_summary(self, pseudo_labels):
        ...
        print(np.array(pseudo_labels).max(),np.array(pseudo_labels).min())
        print(
            f"{clu_num} clusters, "
