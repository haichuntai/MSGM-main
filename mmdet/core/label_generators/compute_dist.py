...
@torch.no_grad()
def build_dist(
    cfg, feat_1, feat_2=None, dist_m=None, verbose=False, k1_new=None, k2_new=None,
):
    """Computes distance.
    ...
    if dist_m == "euclidean":
        ...
        if feat_2 is not None:
            ...
            feat = feat_1
        if k1_new is not None and k2_new is not None:
            dist = compute_jaccard_distance(
                feat, k1=k1_new, k2=k2_new, search_option=cfg.search_type, verbose=verbose,
            )
        else:
            dist = compute_jaccard_distance(
