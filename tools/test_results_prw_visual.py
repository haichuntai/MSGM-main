...
def main(det_thresh=0.05, gallery_size=-1, ignore_cam_id=True, input_path=None):
    ...
    data_root = '/data/thc/data/PRW-v16.04.20/'
    ...
    for probe in probe_set:
        ...
        if (len(det) > 0) and (len(query_gt_box) > 0):
            ...
            print(feat.shape)
            probe_feat.append(np.zeros((256, )))
...
def search_performance_calc(gallery_set, probe_set,
    ...
    for i in range(len(probe_set)):
        ...
        for item in gallery_imgs:
            ...
            count_gt += (gallery_imname in probe_gts) # gallery图像库中真正包含probe行人的场景图像数量
            # compute distance between probe and gallery dets
            if gallery_imname not in name_to_det_feat: # 如果场景中检测出的行人置信度全部小于0.5
                continue
...
#########################################################
            scores = det[:, 4]
            feat_g = scores[:, np.newaxis] * feat_g # 利用检测的置信度分数对检测特征进行重加权
#########################################################

            # compute cosine similarities
            sim = feat_g.dot(feat_p).ravel() # 展开成一维数组

            # assign label for each det
            label = np.zeros(len(sim), dtype=np.int32)
            if gallery_imname in probe_gts: # 遍历全部包含query图像的场景
                gt = probe_gts[gallery_imname].ravel()
                w, h = gt[2] - gt[0], gt[3] - gt[1]
                iou_thresh = min(0.5, (w * h * 1.0) /
                                    ((w + 10) * (h + 10)))
                inds = np.argsort(sim)[::-1] # 降序
                sim = sim[inds]
                det = det[inds]
...
if __name__ == "__main__":
    # for t in [0.05, 0.15, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6]:
    #     print('---------')
    #     print(t)
    #     main(det_thresh=t, input_path=sys.argv[1])
    main(det_thresh=0.5, input_path=sys.argv[1])
