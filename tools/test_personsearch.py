...
def single_gpu_test(model,
    ...
    if not load_gallery:
        ...
        for i, data in enumerate(data_loader): 
            with torch.no_grad():
                #print(data)
                result = model(return_loss=False, rescale=True, **data) # (3, 261)

            batch_size = len(result) # 1
            #print(batch_size)
            if result[0][0].shape[0]>0: # 1
    ...
    try:
        ...
        exit()
        pass
    for i, data in enumerate(query_data_loader): # 1
        if max_iou_with_proposal: # 0
            proposal = data.pop('proposals')[0][0][0].cpu().numpy()
        with torch.no_grad(): # 1
            result = model(return_loss=False, rescale=True, **data) # (1, 261)
            proposal = data['proposals'][0][0][0].cpu().numpy() # (4,)
            scales = data['img_metas'][0].data[0][0]['scale_factor'] # (4,)
        if max_iou_with_proposal: # 0
        ...
        if max_iou_with_proposal: # 0
            ...
            pfeatures.append(result[0][0][:, 5:]) # (1, 256)
...
def main():
    ...
    data_loader = build_dataloader(
        ...
        workers_per_gpu=cfg.data.workers_per_gpu, # 2
        dist=distributed, # False
        shuffle=False)
    
    query_dataset = build_dataset(cfg.data.test) # 6987
    ...
    mmcv.dump([dataset, gboxes, gfeatures, pfeatures], '/data/thc/DICL-main/jobs/cuhk/work_dirs/baseline.pkl')
    sys.exit()

    # 50, 100, 500, 1000, 2000, 4000
    # evaluate_search(dataset, gboxes, gfeatures, pfeatures, threshold=args.reid_threshold, gallery_size=50) # gallery_size=100
    evaluate_search(dataset, gboxes, gfeatures, pfeatures, threshold=args.reid_threshold, gallery_size=100)
    # evaluate_search(dataset, gboxes, gfeatures, pfeatures, threshold=args.reid_threshold, gallery_size=500)
    # evaluate_search(dataset, gboxes, gfeatures, pfeatures, threshold=args.reid_threshold, gallery_size=1000)
    # evaluate_search(dataset, gboxes, gfeatures, pfeatures, threshold=args.reid_threshold, gallery_size=2000)
    # evaluate_search(dataset, gboxes, gfeatures, pfeatures, threshold=args.reid_threshold, gallery_size=4000)
