...
def evaluate_search(
    ...
    """
    assert dataset.num_images == len(gallery_det) # 6978
    assert dataset.num_images == len(gallery_feat)
    assert len(dataset.probes) == len(probe_feat) # 2900

    # TODO: support evaluation on training split
    use_full_set = gallery_size == -1 # False
    fname = "TestG{}".format(gallery_size if not use_full_set else 50) # 'TestG100'
    protoc = loadmat(osp.join(dataset.root_dir, "annotation/test/train_test", fname + ".mat"))[
        fname
    ].squeeze()

    # mapping from gallery image to (det, feat)
    name_to_det_feat = {} # 6974*2*11 表示每张场景图像包含的检测框与行人特征，每张图像的检测框不定
    for name, det, feat in zip(dataset.image_index, gallery_det, gallery_feat):
        scores = det[:, 4].ravel()
        inds = np.where(scores >= threshold)[0] # 0.5
        if len(inds) > 0:
    ...
    accs = []
    topk = [1, 5, 10]
    ret = {"image_root": dataset.data_path, "results": []} # '/data/thc/data/cuhk_sysu/Image/SSM'
    for i in range(len(dataset.probes)): # 2900
        y_true, y_score = [], []
        imgs, rois = [], []
        count_gt, count_tp = 0, 0 # gallery图像库中包含的所有的真实样本，以及检测出来的真实样本，检测结果依据iou判定
        # Get L2-normalized feature vector
        feat_p = probe_feat[i].ravel() # (256,)
        # Ignore the probe image
        probe_imname = str(protoc["Query"][i]["imname"][0, 0][0]) # 's15538.jpg'
        probe_roi = protoc["Query"][i]["idlocate"][0, 0][0].astype(np.int32) # (4,)
        probe_roi[2:] += probe_roi[:2]
        probe_gt = []
        tested = set([probe_imname]) # {'s15538.jpg'}
        # 1. Go through the gallery samples defined by the protocol
        for item in protoc["Gallery"][i].squeeze():
            gallery_imname = str(item[0][0]) # 's15535.jpg'
            # some contain the probe (gt not empty), some not
            gt = item[1][0].astype(np.int32) # (4,)
            count_gt += gt.size > 0
            # compute distance between probe and gallery dets
            if gallery_imname not in name_to_det_feat: # 只判断指定的gallery样本
                continue
            det, feat_g = name_to_det_feat[gallery_imname] # (1, 5)， (1, 256)
            # get L2-normalized feature matrix NxD
            assert feat_g.size == np.prod(feat_g.shape[:2])
            feat_g = feat_g.reshape(feat_g.shape[:2]) # (1, 256)
            # compute cosine similarities
            sim = feat_g.dot(feat_p).ravel() # (1,)
            # assign label for each det
            label = np.zeros(len(sim), dtype=np.int32) # (1,)
            if gt.size > 0:
                w, h = gt[2], gt[3]
                gt[2:] += gt[:2]
                probe_gt.append({"img": str(gallery_imname), "roi": map(float, list(gt))})
                iou_thresh = min(0.5, (w * h * 1.0) / ((w + 10) * (h + 10))) # 0.5
        ...
        recall_rate = count_tp * 1.0 / count_gt # 1.0
        ...
        new_entry = {
            "probe_img": str(probe_imname), # 's17186.jpg'
            "probe_roi": map(float, list(probe_roi)), # (4,)
            "probe_gt": probe_gt,
            "gallery": [],
        }
